...
void A_output(struct msg message) {
  ...
  printf("A: send packet to B  sequence num:%d checksum: %d\n",abs(seqNum),packet.checksum);
  toLayer3(0, packet);
  copy = packet;
  startTimer(0, 15.0);
  }
  else {
    printf("A:  sequence %d is not ACK wait and stop drop message num: %d\n",abs(seqNum),numMsgs);
...
void A_input(struct pkt packet) {
  printf("A:  receive packet from B sequence num:%d, ack num:%d\n",packet.seqNum,abs(packet.ackNum));
  if(packet.ackNum == seqNum && notCorrupt(packet)){
  ...
  printf("A:  this is not expected ACK or check fail! expected ACK:%d,actually:%d \n",+seqNum,packet.ackNum);
  }
}
...
void A_timerInterrupt() {
  printf("A:  timeout send copy to B sequence:%d\n",abs(copy.seqNum));
  toLayer3(0, copy);
  startTimer(0, 15.0);
}
...
void B_input(struct pkt packet) {
  printf("B:  receive packet from A\n");
   int ackNum = seqNum;
  if(packet.seqNum == ackNum && notCorrupt(packet)){
    ...
    printf("B: sent ACK%d to A",abs(ACKpacket.ackNum));
    toLayer3(B, ACKpacket);
    
  }
  else {
    printf("B:  check fail! This is not expected(%d) packet! actually sequence drop it! %d\n",abs(seqNum),abs(packet.seqNum));
  }
}
...
void init() /* initialize the simulator */
{
  ...
  printf("Enter TRACE:");
  scanf("%d", &TRACE);
  srand(9999); /* srand() 用来设置 rand() 产生随机数时的随机数种子。*/
...
void toLayer3(int AorB, struct pkt packet) /* A or B is trying to stop timer */
 {
  ...
  if (TRACE > 2) {
    printf("          toLayer3: seq: %d, ack %d, check: %d ", myPacketPtr->seqNum,
           myPacketPtr->ackNum, myPacketPtr->checksum);
    for (i = 0; i < 20; i++)
